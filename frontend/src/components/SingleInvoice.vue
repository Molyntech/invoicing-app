        ...
        <div class="row">
          <form @submit.prevent="send" class="col-md-12">
            ...
            <div class="form-group">
              ...
              <input type="text" required class="form-control" placeholder="eg Skywalker" v-model="recipient.name">
            </div>

            <div class="form-group">
              <label for="">Recipient Email</label>
              <input type="email" required placeholder="eg moyoa@bcx.co.za" class="form-control" v-model="recipient.email">
            </div>
...
export default {
  ...
  methods: {
    send() {
      ...
      this.loading = "We are sending Invoice, please wait...."
      const formData = new FormData();
      formData.append("user", JSON.stringify(this.user));
      formData.append("recipient", JSON.stringify(this.recipient));
      axios.post("http://localhost:3122/sendmail", formData, {
  ...
  mounted() {
    ...
    axios
      .get(`http://localhost:3122/invoice/user/${this.user.id}/${invoice_id}`, {
        headers: {
